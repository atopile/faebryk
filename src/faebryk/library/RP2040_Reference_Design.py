...
class RP2040_Reference_Design(Module):
    ...
    Based on the official Raspberry Pi RP2040 hardware design guidlines"""
        ...
        # ----------------------------------------
            ...
            flash = SPIFlash()
            led = PoweredLED()
            usb_current_limmit_resistor = L.if_list(2, F.Resistor)
        ...
        self.led.led.color.merge(LED.Color.GREEN)
        self.led.led.brightness.merge(
            TypicalLuminousIntensity.APPLICATION_LED_INDICATOR_INSIDE.value.value
        )

        self.usb_current_limmit_resistor[0].resistance.merge(F.Constant(27 * P.ohm))
        self.usb_current_limmit_resistor[1].resistance.merge(F.Constant(27 * P.ohm))
        ...
        self.usb.usb_if.d.p.connect_via(
            self.usb_current_limmit_resistor[0],
