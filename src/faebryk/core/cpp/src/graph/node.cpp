...
std::string Node::get_root_id() {
    ...
    ss << std::hex << this;
    auto out = ss.str();
    return out.substr(out.size() - 3);
}

std::vector<std::pair<Node *, std::string>> Node::get_hierarchy(bool accept_no_parent) {
    auto p = this->get_parent();
    if (!p && accept_no_parent) {
        return std::vector{std::make_pair(this, this->get_root_id())};
    }
    if (!p) {
        throw NodeNoParent(*this, __func__);
    }
